...
alter table public.chat_messages
  ...
  check (message_type in ('text', 'voice', 'image', 'sticker', 'video', 'announcement'));
...
alter table public.chat_messages
  ...
  check (
    (message_type in ('text', 'announcement') and char_length(message) between 1 and 2000)
    or (message_type not in ('text', 'announcement') and char_length(coalesce(message, '')) <= 2000)
  );
