...
В корне репозитория лежит `Caddyfile`: сайт из `out/` + `sb` → `http://147.45.136.24`.
Пересоберите **существующий** UniqueVocal. Новое платное App не создавать.

Если после деплоя `/__health` всё ещё отдаёт HTML сайта — App Platform Frontend
не читает Caddyfile. Тогда поддержке: пропишите блок на Caddy платформы у UniqueVocal.
