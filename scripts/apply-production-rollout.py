"""Apply production-rollout SQL on the VPS. Requires UVS_SSH_PASS."""
...
SQL_FILES = [
    ROOT / "supabase-migrations" / "2026-09-07-cap-abonement-lessons.sql",
    ROOT / "supabase-migrations" / "2026-09-07-rollout-hardening.sql",
]
...
def main() -> None:
    ...
    try:
        ...
        try:
            for path in SQL_FILES:
                sftp.put(str(path), f"/opt/uvs-migrate/{path.name}")
        finally:
            sftp.close()
        for path in SQL_FILES:
            run(
                ...
                f"-v ON_ERROR_STOP=1 < /opt/uvs-migrate/{path.name}",
            )
        print("production-rollout sql applied", flush=True)
    finally:
        client.close()
